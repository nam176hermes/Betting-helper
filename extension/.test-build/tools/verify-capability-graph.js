...
import { ScriptKind, ScriptTarget, SyntaxKind, createSourceFile, isArrowFunction, isArrayLiteralExpression, isAsExpression, isBinaryExpression, isCallExpression, isBlock, isClassDeclaration, isElementAccessExpression, isExportDeclaration, isFunctionDeclaration, isFunctionExpression, isIdentifier, isImportDeclaration, isIfStatement, isMethodDeclaration, isNewExpression, isNonNullExpression, isNoSubstitutionTemplateLiteral, isObjectBindingPattern, isObjectLiteralExpression, isParameter, isParenthesizedExpression, isPropertyAccessExpression, isPropertyAssignment, isQualifiedName, isReturnStatement, isShorthandPropertyAssignment, isStringLiteral, isSatisfiesExpression, isVariableDeclaration, isVariableStatement, } from "typescript";
...
const OWNER_IMPORTED_HELPERS = new Map([
...
const allFilesBelow = (root) => readdirSync(root).flatMap((name) => {
    const path = resolve(root, name);
    return statSync(path).isDirectory()
        ? allFilesBelow(path)
        : [path];
});
const filesBelow = (root) => allFilesBelow(root).filter((path) => path.endsWith(".ts") || path.endsWith(".js"));
...
const loadCapabilityPolicy = () => {
    try {
        const manifest = parseStrictJson(readFileSync(resolve(process.cwd(), "vendor/hybrid-discovery-v6.3.6/security/discovery-capability-manifest.v1.json")));
        if (!isRecord(manifest)
            || !isRecord(manifest.privileged_chrome_api_policy)
            || !isRecord(manifest.privileged_storage_api_policy))
            return undefined;
        ...
        if (!Array.isArray(methodCalls)
            ...
            || typeof storage.sole_owner !== "string")
            return undefined;
        ...
        const storageMethodSet = new Set();
        ...
        const storageHandlerSet = new Set();
        ...
        if (!equalJson([...chromeOwners.keys()].sort(), expectedChrome))
            return undefined;
        return {
            chromeOwners,
...
export const verifyCapabilityManifest = (manifestPath, canonicalManifestPath) => {
    let actual;
    let canonical;
    try {
...
const constantString = (expression, constants) => {
...
const expressionPath = (expression, constants) => {
    if (isIdentifier(expression))
        return [expression.text];
    if (isParenthesizedExpression(expression)
        || isAsExpression(expression)
...
const normalizeChromeRoot = (path) => path !== undefined
    && ["globalThis", "self", "window"].includes(path[0] ?? "")
    && path[1] === "chrome"
    ? [...path.slice(1)]
    : path === undefined ? undefined : [...path];
const hasChromeRoot = (expression, constants) => {
...
const hasGlobalRoot = (expression) => {
    if (isIdentifier(expression))
        return ["globalThis", "self", "window"].includes(expression.text);
    if (isElementAccessExpression(expression) || isPropertyAccessExpression(expression)) {
...
const pathEndsWith = (path, suffix) => path !== undefined
    && path.length >= suffix.length
    && suffix.every((part, index) => path[path.length - suffix.length + index] === part);
const finalPathPart = (path) => path?.at(-1) ?? "";
const propertyName = (node, constants) => {
    if (isIdentifier(node) || isStringLiteral(node) || isNoSubstitutionTemplateLiteral(node)) {
        return node.text;
    }
    if (node.kind === SyntaxKind.ComputedPropertyName) {
        return constantString(node.expression, constants);
    }
    return undefined;
};
const objectValue = (expression, constants, objects = new Map()) => {
    if (isIdentifier(expression) && objects.has(expression.text))
        return objects.get(expression.text);
    if (isStringLiteral(expression) || isNoSubstitutionTemplateLiteral(expression))
        return expression.text;
...
const classifyCdpMethod = (method) => {
    if (ALLOWED_CDP.has(method))
        return undefined;
    const domain = method.split(".", 1)[0] ?? "";
    if (domain === "Target")
        return "E_TARGET_DOMAIN_DENIED";
    if (domain === "Runtime")
        return "E_CDP_METHOD_DENIED";
    if (FORBIDDEN_CDP_DOMAINS.has(domain))
        return "E_CDP_DOMAIN_DENIED";
    if (domain === "Page" || domain === "Network")
        return "E_CDP_METHOD_DENIED";
    return "E_CDP_METHOD_DENIED";
};
const moduleTarget = (sourcePath, specifier, root) => {
    if (!specifier.startsWith("."))
        return ALLOWED_BARE_IMPORTS.has(specifier) ? specifier : undefined;
...
const pathMatchesOwnerFile = (path, owner) => {
...
const ownerMatches = (node, path, owner) => {
    const [, symbol] = owner.split("#", 2);
    if (symbol === undefined || !pathMatchesOwnerFile(path, owner))
        return false;
    let current = node.parent;
    while (current !== undefined) {
        if (isClassDeclaration(current) && current.name?.text === symbol)
            return true;
        if (isFunctionDeclaration(current) && current.name?.text === symbol)
            return true;
        if (isArrowFunction(current) || isFunctionExpression(current)) {
            if (isFunctionExpression(current) && current.name?.text === symbol)
                return true;
...
const callbackParameters = (expression) => {
...
const compactSource = (node) => node.getText().replace(/\s+/gu, "");
const callbackStartsWithGuard = (expression, exactCondition) => {
    if ((!isArrowFunction(expression) && !isFunctionExpression(expression))
        || !isBlock(expression.body))
        return false;
    const statement = expression.body.statements[0];
    if (statement === undefined || !isIfStatement(statement))
        return false;
    ...
    return condition === exactCondition && (guarded.kind === SyntaxKind.ThrowStatement
        || guardedText === "{thrownewError('SAFETY_STOP');}"
        || guardedText === "{thrownewError(\"SAFETY_STOP\");}"
        || guardedText === "{return;}");
};
const callbackHasExactLocationGuard = (expression, urlPath, offset) => {
    if ((!isArrowFunction(expression) && !isFunctionExpression(expression))
        || !isBlock(expression.body))
        return false;
    ...
    if (declaration === undefined || !isVariableStatement(declaration)
        ...
        || guard === undefined || !isIfStatement(guard))
        return false;
    ...
    if (item === undefined || !isIdentifier(item.name) || item.name.text !== "location"
        ...
        || !pathEndsWith(expressionPath(item.initializer.arguments[0], new Map()), urlPath)) {
        return false;
    }
...
const callbackHasExactStatement = (expression, index, expected) => {
    if ((!isArrowFunction(expression) && !isFunctionExpression(expression))
        || !isBlock(expression.body))
        return false;
...
const callbackStatementCount = (expression) => (isArrowFunction(expression) || isFunctionExpression(expression)) && isBlock(expression.body)
    ? expression.body.statements.length
    : -1;
const hasExactStringSet = (sourceFile, name, expected) => {
    let observed;
    let declarations = 0;
    const inspect = (node) => {
        if (isVariableDeclaration(node)
            ...
            if (node.parent?.parent?.parent === sourceFile
                ...
                const values = node.initializer.arguments[0].elements.map((element) => isStringLiteral(element) ? element.text : undefined);
...
const isAuthenticatedLoopbackOwner = (node, path) => ownerMatches(node, path, "extension/src/security/loopback.ts#AuthenticatedLoopback");
const exactAuthenticatedMessageCallback = (expression) => {
...
const hasEnclosingParameter = (node, name) => {
    let current = node.parent;
    while (current !== undefined) {
        if (isFunctionDeclaration(current)
            ...
            return current.parameters.some((parameter) => isIdentifier(parameter.name) && parameter.name.text === name);
        }
        current = current.parent;
    }
    return false;
};
const hasEnclosingAuthorization = (node, path) => {
    let current = node.parent;
    while (current !== undefined) {
        if (isFunctionDeclaration(current)
            || isFunctionExpression(current)
            || isArrowFunction(current)
            || isMethodDeclaration(current)) {
            const parameter = current.parameters.find((candidate) => isIdentifier(candidate.name) && candidate.name.text === "authorization");
...
const hasAuthorizationTabBinding = (node, path, aliases) => hasEnclosingAuthorization(node, path)
    && pathEndsWith(aliases.get("selectedTabId"), ["authorization", "browser_binding", "tab_id"]);
const exactSelectedTarget = (call, path, expression, constants, aliases) => {
    if (!hasEnclosingAuthorization(call, path))
        return false;
    ...
    return property !== undefined
        ...
        && pathEndsWith(resolveAlias(expressionPath(property.initializer, constants), aliases), ["authorization", "browser_binding", "tab_id"]);
};
const expressionIsEmptyArray = (expression) => expression !== undefined && isArrayLiteralExpression(expression) && expression.elements.length === 0;
const exactExecuteScriptDetails = (call, path, expression, constants, aliases) => {
    if (!hasEnclosingAuthorization(call, path))
        return false;
    if (expression === undefined || !isObjectLiteralExpression(expression))
        return false;
    ...
    if (!equalJson([...properties.keys()].sort(), ["args", "func", "target", "world"]))
        return false;
    const target = properties.get("target");
    if (target === undefined || !isObjectLiteralExpression(target))
        return false;
    ...
    return equalJson([...targetProperties.keys()].sort(), ["frameIds", "tabId"])
        && tabId !== undefined
        && pathEndsWith(resolveAlias(expressionPath(tabId, constants), aliases), ["authorization", "browser_binding", "tab_id"])
        && frameIds !== undefined
        ...
        && constantString(properties.get("world"), constants) === "ISOLATED"
        && func !== undefined
        && isIdentifier(func)
        && ["APP_LOCATION_CONTEXT_V1", "DOM_DOCUMENT_LIFECYCLE_V1"].includes(func.text)
        && expressionIsEmptyArray(properties.get("args"));
};
const hasCanonicalProjectionImport = (sourceFile, path) => {
    if (!/(?:^|[/\\])spool\.(?:ts|js)$/u.test(path))
        return false;
...
const storageTypeFromExpression = (expression, storageTypes, constants) => {
    ...
    if (isIdentifier(expression))
        return storageTypes.get(expression.text);
    ...
    if (isPropertyAccessExpression(expression) && expression.name.text === "result") {
        ...
        if (receiverType === "IDBOpenDBRequest")
            return "IDBDatabase";
        if (receiverType === "IDBRequest")
            return "IDBCursor";
    }
    return undefined;
};
const isRawEvidenceIdentifier = (name) => /^(?:cdpEvent\w*|event\w*|raw(?:Bytes|CdpBytes|CdpEvent|Event|Observation|Payload)\w*)$/u.test(name);
const containsTaintedIdentifier = (node, tainted) => {
    ...
    node.forEachChild((child) => {
        if (!found && containsTaintedIdentifier(child, tainted))
            found = true;
    });
    return found;
};
const isAssignment = (node) => isBinaryExpression(node)
    && ["=", "+=", "-=", "*=", "/=", "%=", "&&=", "||=", "??="].includes(node.operatorToken.getText());
const containsForbiddenFactShape = (node, constants) => {
    if (isObjectLiteralExpression(node)) {
        for (const property of node.properties) {
            if (!isPropertyAssignment(property))
                return true;
            const name = propertyName(property.name, constants);
            if (name === undefined || ["action", "cdp_method", "command"].includes(name))
                return true;
            if (containsForbiddenFactShape(property.initializer, constants))
                return true;
        }
    }
    let forbidden = false;
    node.forEachChild((child) => {
        if (!forbidden && containsForbiddenFactShape(child, constants))
            forbidden = true;
    });
    return forbidden;
};
const hasExactFixedProbeDefinition = (sourceFile, name) => {
    ...
    const inspect = (node) => {
        if (isVariableDeclaration(node)
            ...
            if (node.parent?.parent?.parent === sourceFile
                && node.initializer !== undefined
                && compactSource(node.initializer) === expected)
                exactFound = true;
        }
        node.forEachChild(inspect);
    };
...
const hasExactProbeResultValidator = (sourceFile) => {
...
const exactExecuteScriptResultBinding = (call, sourceFile, functionName) => {
    let initializer = call;
    if (initializer.parent?.kind === SyntaxKind.AwaitExpression)
...
const isWithinStorageHandler = (node, handler, constants) => {
    let current = node.parent;
    while (current !== undefined) {
        if ((isArrowFunction(current) || isFunctionExpression(current))
            ...
            && finalPathPart(expressionPath(current.parent.left, constants)) === handler)
            return true;
        current = current.parent;
    }
    return false;
};
const isWithinSpoolAppendFlow = (node) => {
    let current = node.parent;
    while (current !== undefined) {
        ...
        if (isArrowFunction(current) || isFunctionExpression(current)) {
            ...
            if (assignment === undefined
                ...
                || !STORAGE_HANDLER_NAMES.has(finalPathPart(expressionPath(assignment.left, new Map()))))
                return false;
        }
        if (isIfStatement(current)
            && current.expression.kind === SyntaxKind.FalseKeyword)
            return false;
        current = current.parent;
    }
    return false;
};
const hasExactDatabaseBinding = (sourceFile) => {
    ...
    const inspect = (node) => {
        if (isVariableDeclaration(node)
            ...
            && pathEndsWith(expressionPath(node.initializer.right, new Map()), ["authorization", "browser_binding", "browser_run_id"]))
            databaseBinding = true;
        if (isIfStatement(node)
            && isWithinSpoolAppendFlow(node)
            && compactSource(node.expression)
                === "!LOWERCASE_UUID_V4.test(authorization.browser_binding.browser_run_id)"
            && compactSource(node.thenStatement).includes("thrownewError("))
            uuidGuard = true;
        node.forEachChild(inspect);
    };
    inspect(sourceFile);
...
const hasExactSpoolAppendContract = (sourceFile, path) => {
    let methods = 0;
    let valid = false;
    const inspect = (node) => {
        if (isMethodDeclaration(node) && propertyName(node.name, new Map()) === "append") {
            ...
            const pending = statements.some((statement) => isVariableStatement(statement)
                && statement.declarationList.declarations.some((declaration) => isIdentifier(declaration.name)
            ...
            const findReturns = (candidate) => {
                if (candidate !== node
                    ...
                    return;
                if (isReturnStatement(candidate))
                    returnStatements.push(candidate);
            ...
            valid = node.parameters.length === 1
                ...
                && compactSource(returnStatements[0]) === "returndurableCompletion.promise;";
        }
        node.forEachChild(inspect);
    };
    inspect(sourceFile);
...
const scanSourceFile = (sourceFile, path, root, policy) => {
    const errors = [];
    ...
    const visit = (node) => {
        ...
        if (isPropertyAccessExpression(node) || isElementAccessExpression(node)) {
            const reference = normalizeChromeRoot(resolveAlias(expressionPath(node, constants), aliases));
            ...
            if (reference?.[0] === "chrome" && reference.length >= 2 && !isIntermediate) {
                ...
                if (reference.length === 2) {
                    ...
                    add(reference[1] === "debugger" && sealedDebuggerMembers.has(member)
        ...
        if (isVariableDeclaration(node)) {
            if (isIdentifier(node.name) && node.initializer !== undefined) {
                ...
                if (declaredType?.startsWith("IDB") === true) {
                    ...
                    if (aliasedType !== undefined)
                        storageTypes.set(node.name.text, aliasedType);
                }
                if (/^(?:Document|Element|HTMLElement|HTML\w+Element|Node|Range|Selection)$/u
                    .test(declaredType ?? ""))
                    domTypes.add(node.name.text);
                ...
                const isCanonicalProjection = isCallExpression(node.initializer)
                    && pathEndsWith(expressionPath(node.initializer.expression, constants), ["projectBeforePersistence"])
                ...
                if (structuredValue !== undefined)
                    objects.set(node.name.text, structuredValue);
                ...
                if (isObjectLiteralExpression(node.initializer)
                    && node.initializer.properties.some((property) => (isPropertyAssignment(property) || isShorthandPropertyAssignment(property))
            ...
            if (isIdentifier(node.name)
                ...
                && functionValue.parameters.some((parameter) => isIdentifier(parameter.name) && /^(?:method|command|params)$/u.test(parameter.name.text))) {
        ...
        if (isFunctionDeclaration(node)
            && node.name !== undefined
            && /(?:send|dispatch).*(?:cdp|command)|cdp.*(?:send|dispatch)/iu.test(node.name.text)
            && node.parameters.some((parameter) => isIdentifier(parameter.name) && /^(?:method|command|params)$/u.test(parameter.name.text))) {
            add("E_PRIVILEGED_SINK_OUTSIDE_CLOSED_BROKER");
        }
        if (isCallExpression(node)) {
            if (node.expression.kind === SyntaxKind.ImportKeyword) {
                    ...
                    if (callMember === "chrome.debugger.attach") {
                        ...
                        if (node.arguments.length !== 1
                            ...
                            const funcProperty = details !== undefined && isObjectLiteralExpression(details)
                                ? details.properties.find((property) => isPropertyAssignment(property)
                        ...
                        if (!hasAuthorizationTabBinding(node, path, aliases))
                            add("E_SELECTED_TARGET_BINDING");
                        if (callMember === "chrome.action.onClicked.addListener") {
                            if (node.arguments.length !== 1
                                || !equalJson(parameters, ["tab"])
                                || callbackReturnsValue(callback)) {
                                add("E_CHROME_API_SIGNATURE_MISMATCH");
                            }
                            if (callback === undefined || !callbackStartsWithGuard(callback, "tab.id!==selectedTabId||tab.url===undefined"))
                                add("E_SELECTED_TARGET_BINDING");
                            ...
                            if (callback === undefined || !callbackStartsWithGuard(callback, "sender.id!==\"anfddcancelbmpogbmckmjckjlnpmpjk\""
                            ...
                            if (callback === undefined
                                ...
                                || callbackStatementCount(callback) !== 3
                                || !callbackHasExactStatement(callback, 1, "if(typeofmessage!==\"object\"||message===null"
                                    + "||!PUBLIC_MESSAGE_TYPES.has(message.message_type)"
                                    + "||Object.keys(message).length!==1){return;}")
                                || !callbackHasExactStatement(callback, 2, "sendResponse(handlePublicMessage(message));")) {
                                add("E_UNTRUSTED_COMMAND_CHANNEL");
                            }
                        }
                            ...
                            if (callback === undefined || !callbackStartsWithGuard(callback, "source.tabId!==selectedTabId||source.sessionId!==undefined"
                            ...
                            if (callback === undefined
                                ...
                                || callbackStatementCount(callback) !== 3
                                || !callbackHasExactStatement(callback, 1, "if(!ALLOWED_NETWORK_EVENTS.has(method)){thrownewError(\"SAFETY_STOP\");}")
                                || !callbackHasExactStatement(callback, 2, "projectCdpEvent(method,params);"))
                                add("E_CHROME_API_PARAMETER_MISMATCH");
                        }
                            ...
                            if (!equalJson(parameters, ["details"]) || callbackReturnsValue(callback)) {
                                add("E_CHROME_API_SIGNATURE_MISMATCH");
                            }
                            ...
                            if (isTabReplaced) {
                                ...
                                if (node.arguments.length !== 2
                                    ...
                                    || !callbackStartsWithGuard(callback, "details.tabId!==selectedTabId||details.frameId!==ownedFrameId")
                                    || !callbackHasExactLocationGuard(callback, ["details", "url"], 1)
                                    || !callbackHasExactStatement(callback, 3, "projectNavigationEvent(details);")) {
                                    add("E_CHROME_API_PARAMETER_MISMATCH");
                                }
                            }
                        }
                ...
                if (DESTRUCTIVE_STORAGE_NAMES.has(callFinal)) {
                    ...
                    if (!storageOwner || canonicalStorageMember === undefined) {
                        ...
                        if (callFinal === "createObjectStore") {
                            ...
                            const argumentsValue = node.arguments.map((argument) => objectValue(argument, constants, objects));
                            ...
                            validStorageShape = node.arguments.length === 4
                                && objectValue(node.arguments[2], constants, objects) === false
                                && objectValue(node.arguments[3], constants, objects) === false;
                        }
                ...
                if (pathEndsWith(directPath, ["Reflect", "apply"])) {
                    ...
                    if (DOM_MUTATION_NAMES.has(sinkFinal))
                        add("E_FIXED_PROBE_SHAPE");
                    if (sinkFinal === "postMessage")
                ...
                const projectedAppend = callFinal === "append"
                    ...
                    && pathEndsWith(expressionPath(node.arguments[0].expression, constants), ["projectBeforePersistence"])
                    && hasCanonicalProjectionImport(sourceFile, path);
                ...
                if (pathEndsWith(callPath, ["fetch"]))
                    add("E_OUTBOUND_NETWORK_PATH");
                if (pathEndsWith(callPath, ["sendBeacon"])
                ...
                if (pathEndsWith(callPath, ["postMessage"]))
                    add("E_PAGE_COMMAND_CHANNEL");
                if (callFinal === "send") {
                    ...
                    const authenticated = isAuthenticatedLoopbackOwner(node, path)
                        ...
                        && constantString(payload.arguments[3], constants) === "CLIENT_TO_BACKEND"
                ...
                if (pathEndsWith(callPath, ["onMessageExternal", "addListener"])
                    || pathEndsWith(callPath, ["sendMessage"])) {
                    add("E_UNTRUSTED_COMMAND_CHANNEL");
                }
                if (pathEndsWith(callPath, ["localStorage", "setItem"])
                ...
                if (pathEndsWith(callPath, ["window", "addEventListener"])
                    && node.arguments[0] !== undefined
                    && constantString(node.arguments[0], constants) === "message") {
                    add("E_PAGE_COMMAND_CHANNEL");
                }
                if ((pathEndsWith(callPath, ["chrome", "runtime", "onMessage", "addListener"])
                ...
                if (pathEndsWith(callPath, ["addEventListener"])
                    && node.arguments[0] !== undefined
                    && constantString(node.arguments[0], constants) === "message") {
                    const authenticated = isAuthenticatedLoopbackOwner(node, path)
                        && node.arguments.length === 2
                    ...
                    if (target !== undefined
                        && ((isIdentifier(target) && childTargets.has(target.text))
                            || (isObjectLiteralExpression(target)
                                && target.properties.some((property) => (isPropertyAssignment(property) || isShorthandPropertyAssignment(property))
                                    && ["sessionId", "targetId"].includes(propertyName(property.name, constants) ?? ""))))) {
                        add("E_CHILD_SESSION_DENIED");
                    }
                }
                ...
                if (/frame/iu.test(finalPathPart(callPath))
        ...
        if (isAssignment(node)) {
            if (isIdentifier(node.left)) {
                ...
                if (containsTaintedIdentifier(node.right, tainted))
            ...
            if (STORAGE_HANDLER_NAMES.has(assignmentFinal)) {
                ...
                if (!storageOwner || !canonicalHandler) {
                    ...
                    if (exactEffect === undefined || node.right === undefined
                        ...
                        || !exactEffect.every((effect, index) => callbackHasExactStatement(node.right, index, effect))) {
                        add("E_PERSISTENCE_CONTRACT_MISMATCH");
                    }
                }
            }
            if (pathEndsWith(assignmentPath, ["localStorage"])
        ...
        if (isNewExpression(node)) {
            ...
            if (pathEndsWith(constructorPath, ["Function"]))
                add("E_DYNAMIC_CODE");
            if (pathEndsWith(constructorPath, ["XMLHttpRequest"])
                ...
                || pathEndsWith(constructorPath, ["Image"]))
                add("E_OUTBOUND_NETWORK_PATH");
    ...
    if (storageUsed && pathMatchesOwnerFile(path, policy.storageOwner)) {
        ...
        for (const operation of storageCallOrder) {
            if (operation === requiredOrderedCalls[cursor])
                cursor += 1;
        }
...
const verifyAst = (root, files, policy) => {
    if (files.length === 0)
        return [];
    return unique(files.flatMap((path) => {
        const sourceFile = createSourceFile(path, readFileSync(path, "utf8"), ScriptTarget.Latest, true, extname(path) === ".js" ? ScriptKind.JS : ScriptKind.TS);
        return scanSourceFile(sourceFile, path, root, policy);
    }));
};
export const verifyCapabilityGraph = (root) => {
    const absoluteRoot = resolve(root);
    const policy = loadCapabilityPolicy();
    if (policy === undefined)
        return ["E_CAPABILITY_POLICY_DRIFT"];
    ...
    if (absoluteRoot.endsWith(`${sep}extension${sep}src`)) {
        ...
        const canonical = resolve(process.cwd(), "vendor/hybrid-discovery-v6.3.6/security/discovery-capability-manifest.v1.json");
        if (!existsSync(manifest) || !existsSync(canonical))
            errors.push("E_MANIFEST_CAPABILITY_MISMATCH");
        else
            errors.push(...verifyCapabilityManifest(manifest, canonical));
        const compiledRoot = resolve(absoluteRoot, "../dist");
        if (!existsSync(compiledRoot))
            errors.push("E_COMPILED_CHUNK_MISSING");
        else {
            const expectedCompiled = new Set(filesBelow(absoluteRoot)
                .filter((path) => path.endsWith(".ts"))
                .map((path) => `${relative(absoluteRoot, path).slice(0, -3)}.js`));
            const actualCompiled = new Set(allFilesBelow(compiledRoot).map((path) => relative(compiledRoot, path)));
