...
import { closeClockMapping, reopenClockMapping, selectClockMapping, } from "../../src/contracts/clock-vectors.js";
import { acceptCandidate, evaluateRelease, transitionCoherence, } from "../../src/contracts/clock-coherence.js";
const releaseInput = () => ({
...
void test("closed mappings retain history and cannot reopen or be selected", () => {
    ...
    assert.throws(() => selectClockMapping([{ mapping_id: mappingId, width_us: 1, valid_from_us: 1 }], history), { message: "E_NO_VALID_MAPPING" });
});
...
void test("release admission validates state and epoch IDs with frozen precedence", () => {
    for (const [changes, error] of [
        ...
        const result = evaluateRelease({ ...releaseInput(), ...changes });
