...
export const persistIndexedDbSentinel = async (checkpoint, sentinel = "HD636_INDEXEDDB_SENTINEL") => {
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
    ...
    if (readyIndex >= 0 && readyPath)
        writeFileSync(readyPath, JSON.stringify({ vectorId }));
    if (process.argv.includes("--hold"))
        setInterval(() => undefined, 1000);
    else
        process.stdout.write(JSON.stringify({ vectorId }));
}
