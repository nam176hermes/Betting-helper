...
export const parseStrictJson = (raw) => {
...
export const canonicalContentHash = (artifactType, value, registry) => {
    if (registry === undefined)
        throw new ContractNotImplementedError("F0A-T01");
    const preimage = canonicalPreimage(artifactType, value, registry);
    return crypto.subtle.digest("SHA-256", preimage).then((digest) => Array.from(new Uint8Array(digest), (byte) => byte.toString(16).padStart(2, "0")).join(""));
};
export const verifyCanonicalContentHash = async (artifactType, value, expectedHash, registry) => {
    const computed = await canonicalContentHash(artifactType, value, registry);
    if (computed !== expectedHash)
        throw new Error("CONTENT_HASH_MISMATCH");
};
export const canonicalPreimage = (artifactType, value, registry) => {
    const domains = registry.domains.filter((entry) => entry.artifact_type === artifactType);
    if (domains.length !== 1)
        throw new Error("UNSUPPORTED_VERSION_OR_ALGORITHM");
    ...
    const hashable = Object.fromEntries(Object.entries(projected).filter(([key]) => !excludedKeys.has(key)));
...
const validateRegisteredExclusions = (artifactType, exclusions, registry) => {
    const classes = registry.domain_validation_classes.filter((entry) => entry.artifact_types.includes(artifactType));
    if (classes.length !== 1)
        throw new Error("UNREGISTERED_HASH_EXCLUSION");
    ...
    const groups = registry.schema_binding_groups.filter((entry) => entry.artifact_types.includes(artifactType));
