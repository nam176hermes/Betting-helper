...
interface ClockVectors {
  readonly candidate_acceptance_negative_vectors: ReadonlyArray<{
    readonly expected_state: string;
    readonly id: string;
  }>;
  readonly coherence_state_machine: {
    readonly required_path: readonly string[];
    readonly states: readonly string[];
  };
  readonly golden_mapping: {
    readonly expected: {
      readonly base_uncertainty_us: number;
      readonly network_rtt_us: number;
      readonly offset_interval_us: readonly number[];
      readonly offset_midpoint_us: number;
      readonly padding_us: number;
      readonly raw_lower_us: number;
      readonly raw_upper_us: number;
    };
    readonly source: { readonly resolution_us: number };
    readonly target: { readonly resolution_us: number };
    readonly t1: number;
    readonly t2: number;
    readonly t3: number;
    readonly t4: number;
  };
  readonly release_negative_vectors: ReadonlyArray<{
    readonly expected_state?: string;
    readonly id: string;
  }>;
}
...
const vectors = parseStrictJson(
  readFileSync("vendor/hybrid-discovery-v6.3.6/vectors/clock-coherence-v1.json"),
) as ClockVectors;
...
void test("clock golden mapping and coherence paths reproduce", () => {
  ...
  assert.ok(
    vectors.candidate_acceptance_negative_vectors.every(
      (vector) => vector.expected_state === "WAITING_FOR_RESNAPSHOT",
    ),
  );
  assert.equal(vectors.release_negative_vectors.length, 19);
  assert.equal(
    vectors.release_negative_vectors.find(
      (vector) => vector.id === "RELEASE-NEG-09-MAPPING-CLOSED",
    )?.expected_state,
    "NEW_EPOCH_PENDING",
  );
});
