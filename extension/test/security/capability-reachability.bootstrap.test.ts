...
`;
  ...
  for (const [index, [name, relativePath, body]] of fixtures.entries()) {
    ...
    const compile = spawnSync(
      process.execPath,
      [resolve("extension/node_modules/typescript/bin/tsc"), "-p", configPath],
      { encoding: "utf8" },
    );
...
void test("privileged roots cannot escape through closures or object carriers", () => {
  ...
  writeFileSync(
    resolve(brokerDirectory, "cdp-broker.ts"),
    "export const escape = chrome.debugger.sendCommand;",
  );
  assert.ok(verifyCapabilityGraph(root).includes("E_PRIVILEGED_SINK_OUTSIDE_CLOSED_BROKER"));

  writeFileSync(
    resolve(brokerDirectory, "cdp-broker.ts"),
    "export const escape = (target: chrome.debugger.Debuggee) => chrome.debugger.sendCommand(target, 'Network.disable', {});",
  );
  assert.ok(verifyCapabilityGraph(root).includes("E_PRIVILEGED_SINK_OUTSIDE_CLOSED_BROKER"));
});
...
void test("compiled module set is exact and cannot hide omitted or extra chunks", () => {
  ...
  writeFileSync(
    ...
    readFileSync(resolve("extension/manifest.json")),
  );
...
void test("all canonical manifest escape vectors mutate exact executable surfaces", () => {
  ...
  const actual = JSON.parse(readFileSync(resolve("extension/manifest.json"), "utf8")) as Record<
    string,
    unknown
  >;
  const mutations: ReadonlyArray<readonly [string, (manifest: Record<string, unknown>) => void]> = [
  ...
  for (const [index, [vectorId, mutate]] of mutations.entries()) {
    ...
    assert.deepEqual(
      verifyCapabilityManifest(path, canonicalPath),
      ["E_MANIFEST_CAPABILITY_MISMATCH"],
      vectorId,
    );
  }
});
...
void test("CAP-NEG-037 executes body-class denial against canonical policy", () => {
  ...
  assert.deepEqual(
    verifyCapabilityManifest(resolve("extension/manifest.json"), canonicalPath),
    ["E_BODY_CLASS_NOT_PERMITTED"],
  );
});
...
void test("all seven canonical allowed controls pass source and compiled graph ownership", () => {
  ...
  assert.equal(
    compile.status,
    0,
    `positive capability graph must typecheck and link:\n${compile.stdout}${compile.stderr}`,
  );
  writeFileSync(
    resolve(root, "extension/manifest.json"),
    readFileSync(resolve("extension/manifest.json")),
