...
def test_budget_oracle_concurrent_sequential_and_fallback() -> None:
    for n in [1, 3, 5]:
...
def test_union_and_half_open() -> None:
...
def test_bootstrap_and_missing_quota_are_fail_closed(tmp_path: Any, fake_clock: Any) -> None:
...
def test_every_failed_attempt_and_session_cap(tmp_path: Any, fake_clock: Any) -> None:
...
def test_gap_and_sliding_minute(tmp_path: Any, fake_clock: Any) -> None:
...
def test_provider_reserve_is_fixed_share_not_compounded(tmp_path: Any, fake_clock: Any) -> None:
    with ledger(tmp_path) as q:
        a = reserve(q, fake_clock, "STATUS")
        q.finalize_attempt(
            a.attempt_id,
...
def test_day_rollover_requires_provider_reconciliation(tmp_path: Any, fake_clock: Any) -> None:
    with ledger(tmp_path) as q:
        bootstrap(q, fake_clock)
...
def test_wall_clock_rollback_no_new_allowance(tmp_path: Any, fake_clock: Any) -> None:
...
def test_invalid_quota_headers_block_polling(tmp_path: Any, fake_clock: Any, headers: Any) -> None:
    with ledger(tmp_path) as q:
        a = reserve(q, fake_clock, "STATUS")
        q.finalize_attempt(a.attempt_id, "SUCCESS", headers)
        fake_clock.advance(10)
...
def test_unexplained_remaining_increase_requires_status(tmp_path: Any, fake_clock: Any) -> None:
    with ledger(tmp_path) as q:
        bootstrap(q, fake_clock)
        a = reserve(q, fake_clock)
        q.finalize_attempt(
...
def test_provider_minute_limit_remains_distinct_from_daily(tmp_path: Any, fake_clock: Any) -> None:
    with ledger(tmp_path) as q:
        a = reserve(q, fake_clock, "STATUS")
        q.finalize_attempt(
            a.attempt_id,
...
def test_auth_failure_never_becomes_quota_retry(tmp_path: Any, fake_clock: Any) -> None:
    with ledger(tmp_path) as q:
        a = reserve(q, fake_clock, "STATUS")
        q.finalize_attempt(
...
def test_unchanged_headers_do_not_refill_available_balance(tmp_path: Any, fake_clock: Any) -> None:
    headers = {
        **HEADERS,
        "x-ratelimit-requests-limit": "100",
        "x-ratelimit-requests-remaining": "22",
