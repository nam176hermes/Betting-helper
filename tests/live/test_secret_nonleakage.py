...
def test_repr_and_serialization_are_closed(capsys):
    secret = SecretValue(TEST_SECRET)
    assert repr(secret) == str(secret) == format(secret) == "[REDACTED]"
    for serialize in [json.dumps, pickle.dumps, copy.copy, copy.deepcopy, hash]:
...
def test_invalid_argv_never_echoes_a_secret(capsys):
...
def test_key_removed_from_child_environment_before_dispatch(monkeypatch, capsys):
    import contextlib
    import io
    import os
    ...
    def dispatch(config, secret, receipt):
...
def test_dispatch_error_is_finite(monkeypatch, capsys):
    import contextlib
    import io
