...
EXPECTED_IDS = (
    "SEC_CAPABILITY_MANIFEST", "SEC_CDP_REACHABILITY", "SEC_DYNAMIC_DISPATCH",
    "SEC_TARGET_ESCAPE", "SEC_MESSAGE_SMUGGLING", "SEC_OUTBOUND_NETWORK",
    "SEC_AUTHORIZATION_TRUST", "SEC_REPLAY_REVOCATION", "SEC_CREDENTIAL_EVIDENCE",
    "SEC_CANONICAL_HASHING", "SEC_NO_ARCHIVE", "SEC_BUBBLEWRAP_ISOLATION",
    "SEC_SUPPLY_CHAIN", "SEC_PRODUCTION_BUILD_EXCLUSION",
)
...
def main() -> None:
    ...
    print(
        json.dumps(
            run_review_b_checks(json.loads(args.config.read_text()), json.loads(args.registry.read_text())),
            sort_keys=True,
        )
    )
